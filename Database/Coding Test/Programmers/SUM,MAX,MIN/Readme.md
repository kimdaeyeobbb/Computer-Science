...
### :bookmark: [문제 - 가격이 제일 비싼 식품의 정보 출력하기](https://school.programmers.co.kr/learn/courses/30/lessons/131115)
...
- 다른 쿼리 내부에 포함되어있는 SELECT문을 뜻함
- 서브쿼리는 반드시 괄호로 감싸져 있어야 하며, 서브쿼리를 포함하고 있는 쿼리는 `외부쿼리`, 서브쿼리는 `내부쿼리`라고 부름
- MySQL상 서브쿼리를 포함할 수 있는 쿼리는 SELECT, INSERT, UPDATE, DELETE, SET, DO문이 있음
- 이러한 서브쿼리는 또 다른 서브쿼리 안에 포함될 수 있음
- 서브쿼리에서 사용된 테이블이나 그 결과 집합은 수정할 수 없음
...
- 서브쿼리는 SELECT문의 FROM절에서도 사용할 수 있음
- 이때 서브쿼리에 의해 선택된 결과 집합은 FROM 절에서 하나의 테이블로써 사용됨
- SELECT문의 FROM절에서 사용되는 모든 테이블에는 이름이 필요하다. 따라서 FROM 절에서 사용되는 서브쿼리는 반드시 이름을 정의해야 한다.
